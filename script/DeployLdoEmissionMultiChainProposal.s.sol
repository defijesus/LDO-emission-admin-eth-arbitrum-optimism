...
// Major TODO - figure out how to combine eth mainnet prop into this code
...
contract DeployAllEmission is Script {
  function run() external {
    vm.startBroadcast();

    address[] memory payloads = new address[](3);
    address[] memory targets = new address[](3);

    //Borrow caps payloads:
    payloads[0] = address(0); // opt borrow caps
    targets[0] = AaveGovernanceV2.CROSSCHAIN_FORWARDER_OPTIMISM;

    payloads[1] = address(0); // arb borrow caps
    targets[1] = AaveGovernanceV2.CROSSCHAIN_FORWARDER_ARBITRUM;
